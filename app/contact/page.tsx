        ...
        <Section className="pt-0">
          <PageContainer>
            <div className="grid gap-6 md:grid-cols-2">
              <article className="surface-card p-6"><h2 className="text-2xl font-black">Email</h2><div className="mt-3 grid gap-2 text-[var(--text-secondary)]">{contactEmails.map((email) => <a key={email} className="font-bold text-[var(--aima-deep-blue)]" href={`mailto:${email}`}>{email}</a>)}</div><p className="mt-3 text-[var(--text-secondary)]"><ContentPlaceholder value={content.site.location} fallback="Location will be added soon." /></p></article>
